...
export default function Home() {
  ...
  const [fileDetails, setFileDetails] = useState<{ name: string; type: string }>({
    name: "",
    type: "",
  });
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6">
      <div className="max-w-3xl w-full text-center space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-6">
            ...
            <div className="pt-2">
              <button
                ...
                className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-cyan-400 font-semibold rounded-xl transition"
              >
                Switch to Receiver Camera Mode
              </button>
            </div>
          </div>
        )}
        ...
        {mode === "receiver" && (
          <MatrixReceiver
            sessionId="pixelgo_demo"
            totalChunksExpected={chunks.length || 5}
            base64Key={exportedKey || "AAAA"}
            fileType={fileDetails.type || "application/octet-stream"}
          />
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left pt-6">
          <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-xl space-y-2">
            ...
            <p className="text-xs text-slate-400">
              Files are encrypted locally with AES-256-GCM before optical conversion.
            </p>
          </div>
          <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-xl space-y-2">
            ...
            <p className="text-xs text-slate-400">
              Data streams through high-density RGB matrix flashes read by any camera.
            </p>
          </div>
          <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-xl space-y-2">
            ...
            <p className="text-xs text-slate-400">
              Zero bandwidth costs. Fully compliant with GDPR and Irish DPC rules.
            </p>
          </div>
        </div>
      </div>
